    ...
    <main class="container mx-auto px-4 py-8">
      ...
      >
        ...
        >
          ...
          <div class="p-6">
            <h2
              class="text-xl font-semibold mb-3 text-zinc-900 dark:text-white"
            >
              {{ article.title }}
            </h2>
            <p
              class="text-zinc-600 dark:text-gray-300 mb-4 text-base leading-relaxed"
            >
              {{ article.summary }}
            </p>
      ...
      <div v-else class="text-center py-12">
        <p class="text-zinc-600 dark:text-gray-300 text-xl">
      ...
      >
        <div
          class="bg-white dark:bg-zinc-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto"
        >
          <div class="p-8">
            ...
            <div class="bg-zinc-100 dark:bg-zinc-700 rounded-lg p-6 mb-6">
              ...
              <div
                v-else-if="geminiSummary"
                class="text-zinc-600 dark:text-gray-300 text-lg space-y-4"
              >
                <div
                  class="text-lg space-y-4 text-gray-700 dark:text-gray-300 whitespace-pre-line"
                >
                  {{ geminiSummary }}
                </div>
              </div>
            ...
            <div class="bg-zinc-100 dark:bg-zinc-700 rounded-lg p-6">
              ...
              <div class="flex justify-end space-x-4">
                ...
                >
                  <span>{{
                    geminiLoading ? "Procesando..." : "Preguntar a Gemini"
                  }}</span>
                  <svg
                    v-if="!geminiLoading"
                    xmlns="http://www.w3.org/2000/svg"
                  ...
                  <div
                    v-else
                    class="animate-spin rounded-full h-5 w-5 border-2 border-white"
                  ></div>
                </button>
              </div>
              ...
              >
                ...
                <div class="prose dark:prose-invert max-w-none">
                  <p class="text-zinc-600 dark:text-gray-300 text-lg">
                    {{ geminiResponse }}
                  </p>
                </div>
              </div>
...
async function fetchLatestNews() {
  try {
    ...
    if (data.articles?.length > 0) {
      articles.value = await Promise.all(
        data.articles.map(async (article: any, index: number) => ({
          ...
          image:
            article.urlToImage ||
            "https://via.placeholder.com/800x400?text=IA+News",
      ...
      error.value =
        "No se encontraron noticias de IA. Mostrando contenido de respaldo.";
      useBackupNews();
    }
    ...
    error.value =
      "Error al cargar las noticias de IA. Mostrando contenido de respaldo.";
    useBackupNews();
  } finally {
    loading.value = false;
  }
...
async function generateIntroduction(article: any): Promise<string> {
  const prompt = `
      ...
      Contenido: ${
        article.content || article.description || "No hay contenido disponible"
      }
  ...
  try {
    ...
    return (
      ...
      "No hay información disponible sobre esta noticia."
    );
  }
}
...
async function getGeminiSummary(article: Article) {
  ...
  try {
    ...
    geminiSummary.value =
      "Lo sentimos, no se pudo generar el resumen en este momento. Por favor, intenta de nuevo más tarde.";
  } finally {
...
async function generateDetailedSummary(article: Article): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      "No se pudo generar el resumen. Por favor, intenta de nuevo más tarde."
    );
  }
}
...
async function askGemini() {
  ...
  try {
    ...
    geminiResponse.value =
      "Error al procesar la pregunta. Por favor, intenta de nuevo más tarde.";
  } finally {
    geminiLoading.value = false;
  }
}
  ...
  articles.value = [
    {
      ...
      summary:
        "OpenAI ha anunciado un avance significativo con GPT-4, su último modelo de lenguaje. El sistema ha superado significativamente a sus predecesores en múltiples pruebas de razonamiento y comprensión del lenguaje natural.",
      fullContent:
